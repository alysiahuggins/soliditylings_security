...
echo "Let's get you set up with SolidityLings!"
...
Path=${1:-soliditylings/}
echo "Cloning SolidityLings at $Path..."
git clone -q https://github.com/alysiahuggins/soliditylings_security "$Path"

cd "$Path"

Version=$(curl -s https://api.github.com/repos/alysiahuggins/soliditylings_security/releases/latest | ${PY} -c "import json,sys;obj=json.load(sys.stdin);print(obj['tag_name']) if 'tag_name' in obj else sys.exit(f\"Error: {obj['message']}\");")
...
then
    ...
    then
        ...
        echo "Soliditylings will be installed using the main branch"
        Version="main"
    else
...
echo "Installing the 'soliditylings' executable..."
cargo install --force --path .

if ! [ -x "$(command -v soliditylings)" ]
then
...
echo "All done! Run 'soliditylings' to get started."
